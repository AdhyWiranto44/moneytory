...
<div class="row p-4 w-100 h-100">
    ...
    <div class="col-lg-6">
        ...
        <div class="row">
            <div class="col-md">
                <form action="/roles/{{ $role->name }}/edit" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        <label for="name" class="form-label small mb-1 text-capitalize">nama</label>
                        <input type="text" class="form-control p-3 @error('name') is-invalid @enderror" id="name" name="name" value="{{ $role->name }}" autofocus required>
                        @error('name')
